...
unpackExlistential (Cons a tl) foo = foo a : (unpackExlistential tl foo)
...
unpackEqPair :: (forall a. Eq a => a -> r) -> EqPair -> (r, r)
unpackEqPair f (EqPair x y) = (f x, f y)


-- | b. Write a function that takes a list of 'EqPair's and filters it
-- according to some predicate on the unpacked values.

fltrPairs :: [EqPair] -> (forall a. Eq a => a -> a -> Bool) -> [EqPair]
fltrPairs (p@(EqPair x y) : tl) f = if f x y then p : fltrPairs tl f else fltrPairs tl f
...
--unpack2EqPairs :: EqPair -> EqPair -> (forall a. Eq a => a -> r) -> (forall a. Eq a => a -> r) -> Bool
...
data NestedX input output where
  --NestedX :: (forall subinput. input -> subinput) -> (forall suboutput. suboutput -> output) -> NestedX input output
...
data HTML = HTML { properties :: (String, String), children :: (forall x. Renderable x => [x])}
...
fromNat Z foo = foo SZ
fromNat (S prev) foo = fromNat prev (\t -> foo $ SS t)

counter :: SNat n -> Int -> Int
counter SZ acc = acc
counter (SS prev) acc = counter prev (acc + 1)

test123 = fromNat (S $ S $ S Z) (flip counter 0)
...
filterV :: Vector n a -> (a -> Bool) -> (forall k. Vector k a -> r) -> r
filterV VNil f foo = foo VNil
filterV (VCons head tail) f foo = if f head then filterV (VCons head tail) f foo else filterV tail f foo